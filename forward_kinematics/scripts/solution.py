...
class ForwardKinematics(object):
    ...
    """This function is called every time the robot publishes its joint values. We must use the information we get to compute forward kinematics.
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        # PSEUDO CODE
        # 1. find index by name 
        # use joint value (index) and joint axis to construct transformation matrix T
        #  multiply Tli and T

        #   T = identity matrix
        #   for joint from the first joint to the last joint: 
        #   T = T * transformation matrix of the joint frame
        #   if (the type of the joint equals to revolute):
        #   T = T * transformation matrix caused by the joint rotation
        #   all_transforms.transforms.append(convert_to_message(T, name of the link, 'world_link'))
        ...
        #     tr_trans = numpy.dot(T, tf.transformations.translation_matrix(joints[i].origin.xyz))
                
        #     if joints[i].type == 'revolute':
        #         tr_rot = tf.transformations.quaternion_matrix(tf.transformations.quaternion_about_axis(joint_values.position[index], joints[i].axis))
        #     else:
        #         tr_rot = tf.transformations.identity_matrix()

        #     T = numpy.dot(T, tr_trans, tr_rot)
        #     all_transforms.transforms.append(convert_to_message(T,joints[i].name,'world_link'))
        
        # return all_transforms

        # print(i, joints[i].name)
            # index = joint_values.name.index(joints[i].name)
            # print(index, joints[index].name,joint_values.name[index], joints[index].type, joints[index].axis)
        ...
        #     if joints[i].type == 'revolute':
        #         print(joints[i].axis[0])   
            # print([i], joints[i].name, joints[i].type, joints[i].axis,joints[i].origin.xyz, joints[i].origin.rpy)

    
        # # print('Link_names',link_names)
        # print(joint_values.name)
        # print(joint_values.position)
        # print (joints[1].origin.xyz[2])

        # for i in range(len(joint_values)):
        #     if joints[i].type == 'revolute':
        #         print(joint_values.name[i])
        # # # print(joints[joints.name])
            # print(link_names[i])
            # print('Joints', [i], joints[i].name, joints[i].type, joints[i].axis,joints[i].origin.xyz[1], joints[i].origin.rpy[1])
        # print(len(joint_values.name.index))
        # YOUR CODE GOES HERE
