...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for i in range(len(joints)):
            ...
            # find joint_value index by matching name from joint[i].name
            try:
                index = joint_values.name.index(joints[i].name)
            except ValueError as e:
                # avoid calibration arm error
                rospy.logdebug("%s", e)
            
            # translation matrix is the same for both revolute and fixed joints
            tr_trans = tf.transformations.translation_matrix(joints[i].origin.xyz)
            
            # rotational matrix for revolute joints 
            if joints[i].type == 'revolute':
                ...
                tr_rot = tf.transformations.rotation_matrix(joint_values.position[index], joints[i].axis) 
            else:
                # rotational matrix for fixed joint
                tr_rot = tf.transformations.identity_matrix()
            
            # concatenate or dot
            T = tf.transformations.concatenate_matrices(T, tr_trans, tr_rot)
            # T = numpy.dot(T, tr_trans, tr_rot)

            # parse message to transform
            msg = convert_to_message(T,link_names[i],'world_link')

            # append to all_transforms
